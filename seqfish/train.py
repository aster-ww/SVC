"""
Train SVC on seqFISH+ (NIH/3T3). Cultured cells in small fields of view:
single-scale k=4 neighborhood, kNN within each field of view, D4 augmentation
on, cell identity = cell-cycle phase. Validation = field of view 1.
...
VAL_BATCHES = [1]            # fields of view held out for validation
K_SCALES = (4,)            
K_NB = max(K_SCALES)
USE_CELL_IDENTITY = True     # cell-cycle phase
...
ap.add_argument('--out-dir', default='./output/seqfish',
...
train_data = np.load(f"{dataset_dir}/train_seqfish.npz", allow_pickle=True)
...
tr_ci    = _f32(train_data['identity_label'][train_mask_cells])
...
va_ci    = _f32(train_data['identity_label'][val_mask_cells])
va_batch = batch_of_train[val_mask_cells]

n_train, n_val = tr_image.shape[0], va_image.shape[0]
n_genes = tr_image.shape[1]
n_identity_types = tr_ci.shape[1]
print(f"[seqfish] train={n_train}  val(batches={VAL_BATCHES})={n_val}  "
      f"genes={n_genes}  identity_types={n_identity_types}")
...
class SVC_Dataset_SN(Dataset):
    ...
    morphology masks. Identity and the neighbor context are orientation-invariant."""

    def __init__(self, data_ori, cm, nm, ci, sn_expr, sn_dist, augment=False):
    ...
    def __getitem__(self, i):
        gm = self.data_ori[i]; cm = self.cm[i]; nm = self.nm[i]; ci = self.ci[i]
        ...
        return gm, cm, nm, ci, sn_expr, sn_dist


train_dataset = SVC_Dataset_SN(tr_image, tr_cm, tr_nm, tr_ci,
...
val_ci = torch.from_numpy(va_ci).float().to(device)
...
def compute_val_loss():
    ...
    with torch.no_grad():
        ...
        with torch.autocast('cuda', dtype=torch.bfloat16, enabled=use_amp):
            _ci = val_ci if USE_CELL_IDENTITY else None
            _, mu, r = model(inputs, val_mask, val_sn_expr_t, val_sn_dist_t, val_cm, val_nm, _ci)
        mu = mu.float() * sf_mu
...
print(f"Starting seqfish training "
...
for epoch in range(NUM_EPOCHS):
    ...
    for i, batch in enumerate(train_loader):
        ...
        inputs = inputs * obs_mask_4d
        ci_in = ci if USE_CELL_IDENTITY else None
        with torch.autocast('cuda', dtype=torch.bfloat16, enabled=use_amp):
            _, mu, r = model(inputs, mask, sn_expr, sn_dist, cm, nm, ci_in)
        mu = mu.float() * sf_mu
...
if best_val_state is not None:
    ...
    print(f"\nseqfish done. Best epoch: {best_val_epoch}, "
