"""
Extract per-(cell, gene) latent embeddings for seqFISH+.
...
Usage:
    python extract_latent.py --data-root /path/to/data \
        --ckpt ./output/seqfish/seed2026/checkpoint.pth \
        --out latent_seqfish.npz [--l2-normalize]
"""
...
train_z = np.load(f'{dataset_dir}/train_seqfish.npz', allow_pickle=True)
...
def build(tau):
    m = SVC(gene2vec_weight=gene2vec_weight, n_genes_for_sn=G, k_scales=K_SCALES,
            ...
            use_cell_identity=USE_CELL_IDENTITY, cell_identity_dim=n_identity_types,
...
EMB, CELLNAME, SPLIT = [], [], []
for split in SPLITS:
    z = np.load(f'{dataset_dir}/{split}_seqfish.npz', allow_pickle=True)
