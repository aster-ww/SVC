"""
Extract per-(cell, gene) latent representations for the Xenium breast cancer sections.
...
train_z = np.load(f'{dataset_dir}/train_xenium_breast_cancer.npz', allow_pickle=True)
...
EMB, CELLNAME, SPLIT = [], [], []
for split in SPLITS:
    z = np.load(f'{dataset_dir}/{split}_xenium_breast_cancer.npz', allow_pickle=True)
