...
Usage:
    python evaluate.py --data-root /path/to/data --ckpt-dir ./output/merfish_U2OS
"""
...
ap.add_argument('--ckpt-dir', default='./output/merfish_U2OS',
...
# The checkpoint is either the one train.py wrote into --out-dir, or the released
# checkpoint_MERFISH_U2OS.pth from checkpoints.zip on Zenodo.
RELEASED_CKPT = 'checkpoint_MERFISH_U2OS.pth'
...
test_data  = np.load(f"{dataset_dir}/test_merfish_U2OS.npz")
train_data = np.load(f"{dataset_dir}/train_merfish_U2OS.npz")
...
print(f"[merfish_U2OS] Test cells: {test_image.shape[0]}, Genes: {len(gene_names)}")
...
class SVC_TestDataset_SN(Dataset):
    def __init__(self, data_ori, cm, nm, sn_expr, sn_dist):
    ...
    def __getitem__(self, i):
        return (self.data_ori[i], self.cm[i], self.nm[i],
    ...
    for selected_gene in groups:
        ...
        with torch.no_grad():
            for batch in val_loader:
                inputs_ori, cm, nm, sn_expr, sn_dist = batch
